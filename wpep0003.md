...
Existing solutions: [JWT](https://jwt.io/) and [Fernet tokens](https://docs.openstack.org/keystone/pike/admin/identity-fernet-token-faq.html).

Instead of generating a UUID as authentication token, wazo-auth can generate a base64-encoded string containing the token metadata and a signature that can be validated from a public key. This token is passed in each HTTP request in a header (Authorization or other).

When a daemon receives an HTTP request, it:

1. extracts and decodes the token
2. validates the token signature against the public key
3. verifies the permissions
4. processes the HTTP request

This requires an asymetric key pair to be generated for wazo-auth, and the public key to be deployed in all other daemons.

Pros:

* Removes the need for HEAD/GET /token/uuid

Cons:

* All HTTP requests get larger, due to the size of the token, especially if containing the full list of ACL.
* No more revocation is possible (not necessary for internal tokens, and expiration may be sufficient)
